import 'server-only'
import { kpis, anomalias, indiceHhi } from './queries'
import { projecao } from './forecast'
import { brl, rotuloMes } from './format'
...
export async function alertas(sessao) {
  const [k, base, desvios, hhi] = await Promise.all([
    kpis(sessao), projecao(sessao, 6), anomalias(sessao, 4, 3), indiceHhi(sessao),
  ])
  const lista = []
  ...
  if (hhi?.hhi !== null && Number(hhi?.hhi) >= 0.25) {
    lista.push({
      ...
      texto: `Índice HHI de ${Number(hhi.hhi).toFixed(3)}, acima do limite usual de 0,25.`,
      href: '/indicadores',
    })
  }
