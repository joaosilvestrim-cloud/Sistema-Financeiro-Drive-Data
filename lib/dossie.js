...
export async function montarDossie(sessao, competencia = null) {
  ...
  const [kpis, meses, aging, clientes, categorias, desvios, prazos, cobertura, hhi, fluxo] = await Promise.all([
  ...
  const previstos = fluxo && fluxo.meses
    ...
    .reduce((acc, m) => ({ entradas: acc.entradas + m.entradas, saidas: acc.saidas + m.saidas }),
      { entradas: 0, saidas: 0 })
