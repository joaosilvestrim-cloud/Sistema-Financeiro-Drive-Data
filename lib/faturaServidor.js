...
export async function enviarFatura(sessao, { conexaoId, vencimento, itens, contaExternalId, pessoaExternalId }) {
  ...
  // A conexão veio pelo navegador e precisa ser conferida contra a sessão antes
  // de qualquer coisa.
  //
  // O `prepararFatura` escolhe a conexão do próprio tenant, mas o valor volta
  // pela tela e um cliente pode mandar outro. Sem esta conferência, quem
  // soubesse o id de uma conexão alheia lançaria despesa dentro do Conta Azul
  // de outra empresa, usando o token OAuth dela: `clientFor` busca o token pelo
  // id da conexão e não pergunta de quem ela é.
  //
  // Não é fácil de explorar, porque o id é um UUID. Também não é caro conferir.
  const dono = await q1(
    `select id from core.connection where id = $1 and tenant_id = $2`,
    [conexaoId, sessao.tenantId],
  )
  if (!dono) return { erro: 'Conexão inválida para esta conta.' }
