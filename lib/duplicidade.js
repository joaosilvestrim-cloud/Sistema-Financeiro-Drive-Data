...
//
// Regra do Diogo (reunião de 11/09): "um baixado e outro aberto" também é
// sinal forte. Se fossem duas parcelas legítimas, as duas seguiriam a mesma
// vida. Uma paga e a gêmea parada em aberto é o padrão clássico do lançamento
// que entrou duas vezes: alguém pagou a de verdade e a cópia ficou para trás,
// esperando ser paga de novo.
  ...
  const grupos = await q(
    `with candidatos as (
       select
         ...
         count(*) filter (where coalesce(i.nao_pago, 0) <= 0.009) as baixados,
         count(*) filter (where coalesce(i.nao_pago, 0) > 0.009)  as abertos,
     ...
     select c.*, p.nome as pessoa,
       ...
       + (case when c.baixados > 0 and c.abertos > 0 then 60 else 0 end)
...
function motivoDe(g) {
  ...
  const meioAMeio = Number(g.baixados) > 0 && Number(g.abertos) > 0
  if (minutos !== null && minutos < MINUTOS_CLIQUE_DUPLO) {
    const base = minutos < 1
      ? 'criados no mesmo minuto, com a mesma data e o mesmo valor'
      : `criados com ${Math.round(minutos)} minutos de diferença`
    return meioAMeio ? `${base}, e um já foi baixado enquanto o outro segue em aberto` : base
  }
  if (meioAMeio) return 'um já foi baixado e o outro segue em aberto, com a mesma data e o mesmo valor'
