...
function situacao(membro, conexoes) {
  ...
  const restaMs = emTeste ? new Date(membro.trial_ate) - agora : null
  // Arredonda para cima. No ultimo dia a tela precisa dizer um dia, nao zero.
  const diasRestantes = restaMs === null ? null : Math.max(0, Math.ceil(restaMs / 86400000))
  const testeVencido = emTeste && restaMs <= 0
  ...
  const acessoPago = membro.acesso_ate && new Date(membro.acesso_ate) > agora
