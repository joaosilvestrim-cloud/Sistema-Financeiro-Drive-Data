...
export async function conciliacao(sessao) {
  ...
  // Pendente conta a partir do dia em que a empresa começou a conciliar.
  //
  // Achado do Diogo olhando a tela em 11/09: "não temos 262 pendentes". Ele
  // tinha razão nos dois sentidos. As 262 baixas sem marca de conciliação
  // existem, mas 203 delas são de antes de julho de 2025, quando ninguém ali
  // usava a conciliação do Conta Azul. Cobrar conciliação de uma época em que
  // ela não era praticada não mede a equipe, mede a arqueologia.
  //
  // O corte é a primeira baixa conciliada da própria empresa, não uma data
  // fixa: cada cliente começou quando começou. O que vem antes é reportado à
  // parte como legado, porque sumir com número também não é opção.
  const resumo = await q1(
    `with inicio as (
       select min(s.data_pagamento) as dia
         from core.settlement s
        where ${where} and s.reconciliacao_external_id is not null
     )
     select
       (select dia from inicio)                                          as inicio_uso,
       count(*) filter (where s.reconciliacao_external_id is null
                          and s.data_pagamento >= (select dia from inicio)) as pendentes,
       coalesce(sum(s.valor) filter (where s.reconciliacao_external_id is null
                          and s.data_pagamento >= (select dia from inicio)), 0) as valor_pendente,
       count(*) filter (where s.reconciliacao_external_id is null
                          and s.data_pagamento < (select dia from inicio))  as legado_titulos,
       coalesce(sum(s.valor) filter (where s.reconciliacao_external_id is null
                          and s.data_pagamento < (select dia from inicio)), 0) as legado_valor,
       ...
       min(s.data_pagamento) filter (where s.reconciliacao_external_id is null
                          and s.data_pagamento >= (select dia from inicio)) as mais_antiga_pendente
     from core.settlement s
     where ${where}`,
    params,
  )
  ...
  const faixas = await q(
    `select
     ...
     where ${where} and s.reconciliacao_external_id is null and s.data_pagamento is not null
       and s.data_pagamento >= coalesce((
         select min(x.data_pagamento) from core.settlement x
          where x.tenant_id = s.tenant_id and x.reconciliacao_external_id is not null
       ), s.data_pagamento)
     group by 1`,
    params,
  )
...
// Os títulos em aberto, um a um, com a faixa de atraso já calculada.
//
// A expressão da faixa é copiada de mart.aging_snapshot de propósito. Se as
// duas divergirem, o total da faixa deixa de bater com a soma do que abre
// dentro dela, que é o jeito mais rápido de perder a confiança de quem confere.
export async function titulosPorFaixa(sessao, kind) {
  const { where, params } = escopo(sessao, 'i')
  return q(
    `select
       case
     ...
     order by i.data_vencimento`,
    [...params, kind],
  )
}

// Os títulos em aberto de cada pessoa, para abrir dentro da linha dos dez
// maiores. Uma consulta só para todo mundo, agrupada depois na tela: dez
// consultas separadas dariam o mesmo resultado com dez idas ao banco.
export async function titulosPorPessoa(sessao, kind) {
  const { where, params } = escopo(sessao, 'i')
  return q(
    `select
       coalesce(p.nome, 'Sem cadastro')  as pessoa,
...
// Títulos que a empresa desistiu de receber.
//
// Achado de uma varredura de integridade: oito títulos com `total` preenchido,
// `pago` zero e `nao_pago` zerado pelo ERP. Não é defeito da ingestão, é o
// comportamento certo de uma baixa por perda: o valor sai do que há a receber
// porque ninguém mais espera por ele.
//
// O problema é o que acontece depois. O DRE soma `total` por competência, como
// tem que ser em regime de competência, então a receita perdida continua lá. A
// perda correspondente não existe em lugar nenhum, porque o ERP não lança uma
// despesa quando alguém marca "perdido". Resultado: o DRE fica otimista pelo
// valor exato do que foi perdido, e nenhuma tela dizia isso.
//
// São R$ 18 mil na base da DriveData, 1,2% da receita de doze meses, e
// crescendo: 3,5 mil em abril, 7 mil em maio, 5,1 mil em junho. É pouco no
// total e é muito como tendência, que é justamente o tipo de número que só
// aparece se alguém for procurar.
export async function perdas(sessao, meses = 12) {
  const { where, params } = escopo(sessao, 'i')
  const linhas = await q(
    `select
       to_char(date_trunc('month', coalesce(i.data_competencia, i.data_vencimento)), 'YYYY-MM') as mes,
       coalesce(p.nome, 'Sem cadastro')  as pessoa,
       i.descricao,
       i.data_vencimento,
       i.total
     ...
     where ${where}
       ...
       and i.status = 'PERDIDO'
       and coalesce(i.data_competencia, i.data_vencimento)
             >= date_trunc('month', current_date) - make_interval(months => $${params.length + 1})
     order by i.data_vencimento desc`,
    [...params, meses],
  )

  const total = linhas.reduce((a, l) => a + Number(l.total), 0)
  const porMes = new Map()
  for (const l of linhas) porMes.set(l.mes, (porMes.get(l.mes) ?? 0) + Number(l.total))

  // Compara os três últimos meses com os três anteriores. Perda é rara por
  // definição, então o que interessa não é o mês, é se está acelerando.
  const ordem = [...porMes.entries()].sort()
  const ultimos = ordem.slice(-3).reduce((a, [, v]) => a + v, 0)
  const anteriores = ordem.slice(-6, -3).reduce((a, [, v]) => a + v, 0)

  return {
    total,
    titulos: linhas.length,
    linhas,
    porMes: ordem.map(([mes, valor]) => ({ mes, valor })),
    acelerando: ultimos > anteriores && ultimos > 0,
    ultimos,
    anteriores,
  }
}
