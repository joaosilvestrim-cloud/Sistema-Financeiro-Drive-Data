...
export async function estruturaCusto(sessao, meses = MESES_PADRAO) {
  ...
  // As categorias de cada classe, para abrir dentro da linha. A pergunta que
  // vem depois de "custo fixo é 34% da receita" é sempre qual conta pesa nesses
  // 34%, e ela já está na mesma varredura que produziu o total.
  const categoriasDe = (classe) => {
    const mapa = new Map()
    for (const l of linhas) {
      if (l.classe !== classe) continue
      const nome = l.categoria ?? 'Sem categoria'
      if (!mapa.has(nome)) mapa.set(nome, { categoria: nome, valor: 0, titulos: 0, manual: l.manual })
      const c = mapa.get(nome)
      c.valor += Number(l.valor ?? 0)
      c.titulos += Number(l.titulos ?? 0)
    }
    const total = [...mapa.values()].reduce((a, c) => a + c.valor, 0)
    return [...mapa.values()]
      .sort((a, b) => b.valor - a.valor)
      .map((c) => ({ ...c, participacao: total > 0 ? c.valor / total : null }))
  }
  ...
  return {
    ...
    categorias: {
      receita: categoriasDe('receita'),
      direto: categoriasDe('direto'),
      variavel: categoriasDe('variavel'),
      fixo: categoriasDe('fixo'),
      // Parcela sem categoria nenhuma não aparece no agrupamento acima, mas
      // entra no total de não classificado. Sem esta linha a soma do detalhe
      // não fecharia com a da linha fechada.
      naoClassificado: [
        ...categoriasDe(null),
        ...(Number(semCategoria?.valor ?? 0) > 0
          ? [{
              categoria: 'Sem categoria no ERP',
              valor: Number(semCategoria.valor),
              titulos: Number(semCategoria.titulos),
              participacao: null,
            }]
          : []),
      ].sort((a, b) => b.valor - a.valor),
    ...
    // varia com a venda, ou seja, o custo direto e o imposto. É com ela que a
    // empresa paga a estrutura.
    margemContribuicao: receita > 0 ? 1 - (variavel + direto) / receita : null,

    // Ponto de equilíbrio: quanto precisa faturar para o resultado ser zero.
    // Custo fixo dividido pela margem de contribuição, e não pela sobra que
    // gera o multiplicador. São contas diferentes e confundir as duas dá um
    // número que parece certo e não é.
    pontoEquilibrio: (() => {
      const mc = receita > 0 ? 1 - (variavel + direto) / receita : null
      return mc !== null && mc > 0 ? fixo / mc : null
    })(),

    // Resultado operacional. É EBITDA gerencial e não EBITDA contábil: aqui
    // não entram juros, investimento nem movimento não operacional, porque
    // esses caem na classe 'fora'. O imposto sobre venda entra, porque no
    // Simples ele é percentual do faturamento e some junto com a receita.
    ebitda: receita - direto - variavel - fixo,
    margemEbitda: receita > 0 ? (receita - direto - variavel - fixo) / receita : null,
...
//
// Não é rentabilidade por produto, que é o que o Diogo pediu, mas é o mais perto
// que a base chega hoje. Os centros de custo da DriveData são áreas
// funcionais, e alguns deles, como "Consultoria de BI e Dados", andam junto com
// a linha de serviço. Onde o centro de custo for a linha de negócio, esta tabela
// já responde onde o dinheiro entra e onde ele sai.
//
// O que não tem centro de custo aparece como linha própria, e não rateado. Na
// base da DriveData isso é perto de 7% dos lançamentos, e diluir esse pedaço nas
// outras linhas mudaria o resultado de cada uma sem ninguém perceber.
export async function resultadoPorCentro(sessao, meses = MESES_PADRAO) {
  const { where, params } = escopo(sessao, 'i')
  return q(
    `select coalesce(cc.nome, 'Sem centro de custo')                     as centro,
            sum(i.total) filter (where i.kind = 'receivable')            as receita,
            sum(i.total) filter (where i.kind = 'payable')               as despesa,
            coalesce(sum(i.total) filter (where i.kind = 'receivable'), 0)
              - coalesce(sum(i.total) filter (where i.kind = 'payable'), 0) as resultado,
            count(*)                                                     as titulos
       from core.installment i
