...
export async function dre(sessao, granularidade = 'mes') {
  ...
  for (const l of linhas) {
    ...
    // A mesma varredura monta o que abre dentro da linha. O grupo do DRE é uma
    // caixa com nomes dentro, e a primeira pergunta de quem vê "Despesa
    // operacional subiu 18%" é qual categoria subiu. A resposta já está nesta
    // lista: pedir de novo ao banco a cada clique seria pagar duas vezes pelo
    // mesmo dado.
    if (!g.categorias) g.categorias = new Map()
    if (!g.categorias.has(l.categoria)) {
      g.categorias.set(l.categoria, { categoria: l.categoria, porPeriodo: {}, total: 0 })
    }
    const c = g.categorias.get(l.categoria)
    c.porPeriodo[l.periodo] = (c.porPeriodo[l.periodo] ?? 0) + Number(l.total)
    c.total += Number(l.total)
  }

  // O Map vira lista ordenada por peso, que é a ordem em que alguém procura.
  for (const g of grupos.values()) {
    g.categorias = [...g.categorias.values()].sort((a, b) => b.total - a.total)
  }
