...
export async function assinatura(tenantId) {
  ...
  const restamMs = emTeste ? new Date(t.trial_ate) - Date.now() : null
  // Arredonda para cima: no último dia a tela deve dizer "1 dia", não "0".
  const diasRestantes = restamMs === null ? null : Math.max(0, Math.ceil(restamMs / 86400000))

  return {
    ...t,
    emTeste,
    diasRestantes,
    testeVencido: emTeste && restamMs <= 0,
