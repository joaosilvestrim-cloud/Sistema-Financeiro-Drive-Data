...
import { aceitarConvite } from './equipe.js'
...
export async function garantirConta(user, { empresa, origem } = {}) {
  ...
  // Convidado não ganha tenant próprio: ele entra na empresa de quem o
  // chamou. O casamento é por token do convite ou pelo e-mail que o Supabase
  // confirmou, e acontece aqui porque este é o único ponto por onde todo
  // primeiro login passa.
  const convite = await aceitarConvite(user)
  if (convite) return { tenantId: convite.tenantId, criada: false }
