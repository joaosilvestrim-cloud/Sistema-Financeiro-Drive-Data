...
import { mascarar, revelarEmTudo } from './anonimo.js'
...
const INDICADORES = {
  ...
  // Os cartoes das telas de Indicadores e Fluxo de caixa, alem dos tiles.
  curva_saldo: 'A curva do saldo em conta, medida ate hoje e projetada adiante. Use projecao_de_caixa.',
  movimento: 'Entradas e saidas de caixa mes a mes nos ultimos doze meses. Use fluxo_de_caixa_realizado_por_mes.',
  composicao_previsto: 'Quanto da entrada projetada ja esta lancada no ERP e quanto e apenas estimativa.',
  contas: 'Como o saldo esta distribuido entre as contas financeiras.',
  sazonalidade: 'Indice sazonal da receita por mes do ano. Acima de 1 e mes forte, abaixo e mes fraco.',
  pareto: 'Concentracao do faturamento nos maiores clientes, com o acumulado.',
  desvios: 'Categorias que fugiram do proprio padrao historico nos ultimos meses.',
  crescimento_real: 'Receita nominal contra receita deflacionada pelo indice informado.',
}
...
function referenciaDe(sessao, dossie) {
  // Normalizado para ISO antes de comparar. A mesma data chega como objeto Date
  // vindo de uma consulta e como texto vindo de um json_agg, e o sort() coage
  // Date para o formato local do sistema. O resultado eram duas referencias
  // diferentes para o mesmo instante, e o cache nunca acertava entre a tela e a
  // linha de comando.
...
async function gerar(sessao, dossie, referencia) {
  // Nome de cliente e de conta bancaria viram apelido antes de sair. O nome
  // real volta no fim, aqui dentro, e nunca cruza a fronteira.
  ...
  const { texto, modelo, tokens } = await conversar([
    ...
    // Vinte e dois bullets mais o raciocinio do modelo nao cabem em 2500. Com
    // teto curto o JSON volta truncado e o parse quebra, o que aparece como
    // "nenhum bullet" sem explicacao nenhuma.
  ], { maxTokens: 6000 })
  ...
  const filtrados = Object.fromEntries(
    Object.entries(bullets)
      ...
      // O nome real volta aqui, no servidor. A Groq só viu o apelido.
      .map(([k, v]) => [k, revelarEmTudo(v.trim(), mapa)]),
  )
...
export const insights = cache(async function insights(sessao) {
  // Desligada pelo cliente quer dizer que nada sai daqui. E a promessa da
  // politica de privacidade virando codigo, no unico lugar que importa.
  if (!process.env.GROQ_API_KEY || sessao.conta?.iaHabilitada === false) return {}
  try {
    ...
    // A leitura da IA é um complemento: se falhar, os indicadores continuam na
    // tela sem o bullet e ninguém fica sem o número por causa disso. Mas o
    // motivo vai para o log, senão a falha é invisível e indiagnosticável.
